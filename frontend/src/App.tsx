...
export default function App() {
    ...
    const codeKey = (language: string, folder: string) => `${language}_${folder}`;
    ...
    const loadProblem = (p: Problem) => {
        fetch(`${API}/problems/${p.language}/${p.folder}`)
            ...
            .then(data => {
                setSelected(data);
                setCode(savedCode[codeKey(data.language, data.folder)] || data.starter_code || "");
                clearResults();
                setShowHints(false);
                setHintIndex(0);
            });
    ...
    const handleRun = async () => {
        if (!selected) return;
        setSavedCode(prev => ({ ...prev, [codeKey(selected.language, selected.folder)]: code }));
        setRunning(true);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (data.passed) {
                const newProgress = { ...progress, [codeKey(selected.language, selected.folder)]: true };
                setProgress(newProgress);
                setProblems(prev => prev.map(p => p.folder === selected.folder && p.language === selected.language ? { ...p, completed: true } : p));
    ...
    const handleSkip = () => {
        ...
        const idx = filtered.findIndex(p => p.folder === selected.folder && p.language === selected.language);
    ...
    const completedCount = problems.filter(p => progress[codeKey(p.language, p.folder)] || p.completed).length;
                ...
                {activePanel === "problems" && (
                    <div className="panel-content">
                        {!selected ? (
                            <>
                                ...
                                <div className="problem-list">
                                    ...
                                    {filteredProblems.map(p => (
                                        <div key={codeKey(p.language, p.folder)} className={`problem-card ${(progress[codeKey(p.language, p.folder)] || p.completed) ? "completed" : ""}`}
                                            onClick={() => loadProblem(p)}>
                                            <div className="problem-card-left">
                                                <span className="completion-dot">{(progress[codeKey(p.language, p.folder)] || p.completed) ? "✓" : "○"}</span>
                                            </div>
                            ...
                            <>
                                <button className="back-btn" onClick={() => { setSavedCode(prev => ({ ...prev, [codeKey(selected.language, selected.folder)]: code })); setSelected(null); clearResults(); }}>← Back</button>
                                {(progress[codeKey(selected.language, selected.folder)] || selected.completed) && (
                ...
                <div className="editor-area">
                    ...
                    <div className="editor-wrapper">
                        <Editor
                            ...
                            onChange={val => {
                                setCode(val || "");
                                if (selected) setSavedCode(prev => ({ ...prev, [codeKey(selected.language, selected.folder)]: val || "" }));
                            }}
                            theme="vs-dark"
                            options={{
